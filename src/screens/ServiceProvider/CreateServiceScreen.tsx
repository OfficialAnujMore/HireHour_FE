...
const CreateServiceScreen = () => {
  ...
  const handleServiceCreation = async () => {
    ...
    if (!selectedDate || selectedDate.timeSlots.every(slot => slot.available)) {
      Alert.alert(
        'Error',
        'Please select a date and at least one available time slot.',
      );
      return;
    }

    const selectedSlots = selectedDate.timeSlots.filter(slot => slot.available);
    const finalSchedule = dates
      .map(item => {
      ...
      const data = {
        ...
        serviceData: {
          ...
          servicePreview: [
            {
              imageUri:
                'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSWMrZ357DGddYlOKJJpLwZ6nPsEJdH3RYFOWGoX00-6UumyVwUYQNAZBEBlU23tngDdqU&usqp=CAU',
            },
            {
              imageUri:
                'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTDQjNpHNzb6bZBzjYnv65_tjUwn-fdIvs_-eWmjzHcAftNsBXdaOLb-9Mb8SsmQK9WRb0&usqp=CAU',
            },
            {
              imageUri:
                'https://static.vecteezy.com/system/resources/thumbnails/030/633/119/small_2x/an-artistic-composition-featuring-an-acoustic-guitar-amidst-nature-with-space-for-text-with-a-textured-background-of-leaves-or-tree-bark-ai-generated-photo.jpg',
            },
          ],
