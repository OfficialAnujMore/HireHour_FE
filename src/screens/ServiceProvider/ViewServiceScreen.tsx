...
const ViewServiceScreen = (props:any) => {
    ...
    return (
        <ScrollView style={styles.container}>
            ...
            <CustomText label={`Description: ${item.description}`} style={styles.text} />
            <CustomText label={`Charges Per Hour: $${item.chargesPerHour}`} style={styles.text} />

            <CustomText label="Service Images" style={styles.subHeading} />
            <View style={styles.imageContainer}>
                {item.servicePreview.map((image, index) => (
                    <Image key={index} source={{ uri: image.imageUri }} style={styles.image} />
                ))}
            </View>
            ...
            {item.schedule.map((schedule, index) => (
                <View key={index} style={styles.scheduleItem}>
                    <CustomText label={`${schedule.day}, ${schedule.date} ${schedule.month}`} />
                    <CustomText label={`Time Slots: ${schedule.timeSlots.map((slot) => slot.time).join(", ")}`} />
                </View>
            ))}
        </ScrollView>
    );
